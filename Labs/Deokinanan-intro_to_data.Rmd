...
##### Answer
There are 20000 cases in this data set. The variables are:

|general health|exercised?|health plan?|smoke 100?|height|weight|desired weight|age|gender|
|---|---|---|---|---|---|---|---|---|
|genhlth|exerany|hlthplan|smoke100|height|weight|wtdesire|age|gender|  
|categorical|categorical|categorical|categorical|discrete|discrete|discrete|continuous|categorical|  
...
##########################################
# Number of males in are:
sum(cdc$gender=="m")

##########################################
# The proportion of the sample reports being in excellent health, in percent form, is
...
##### Answer
The mosaic plot shows that there are more females that do not smoke more than 100 cigarettes that their male counterparts. In addition, there is also more females that do not smoke more than 100 cigarettes than the female that do smoke at least 100 cigarettes.
...
```{r 6th-column, eval=FALSE, include=FALSE}
cdc[,6]
```
...
```{r weight, eval=FALSE, include=FALSE}
cdc$weight
```
...
```{r true-male, eval=FALSE, include=FALSE}
cdc$gender == "m"
```

or

```{r true-over30, eval=FALSE, include=FALSE}
cdc$age > 30
```
...
##### Answer
It is noticable from the box plots that as the rating for general health differs from *excellent* to *poor*, the median gets larger. This suggests that a larger BMI is more apparent in participants who rate their general health as poor.

Take a look at BMI and whether a participant exercised in the past month or not, `exerany`.
```{r}
boxplot(bmi ~ cdc$exerany)
```

Given that BMI accounts for the ratio of tissues mass (fat and muscle) of a body based on height, and the muscle weighs more than fat, looking at whether or not particiants exercise would provide insights on the effects of exercising on the BMI by seeing how much their BMI differs. From the boxplot, the median BMI of those who exercise is less than those who do not, suggesting that they have a lower ratio of weight to height in those who exercise. 
...
##### Answer
```{r}
plot(cdc$weight,cdc$wtdesire, main="Scatter plot of weight vs desired weight", xlab="weight", ylab="desired weight")

# For better views
plot(cdc$weight[1:1000],cdc$wtdesire[1:1000], main="Scatter plot of weight vs desired weight", xlab="weight", ylab="desired weight")
plot(cdc$weight[5258:7500],cdc$wtdesire[5258:7500], main="Scatter plot of weight vs desired weight", xlab="weight", ylab="desired weight")
```

With 20000 data points, the scatter plots look cluttered, therefore looking at various ranges, it is still clear that there are two apparent relationships. Firstly, there is a positive, linear relationship in the beginning between these two variables, that is, as the actual weight increase, the desired weight increases too, or rather close to a 1 to 1 relationship, meaning some participants' current weight is the desired weight. But along the ends in particular, as the actual weight increases, the desired weight decreases.
...
##### Answer
`wdiff` is a numeric data type containing intergers. An observation of 0 means that the person's weight has met their desired weight and they are okat with their actual weight. If the `wdiff` is positive, it means that the person wishes to gain more weight upon their actual weight, whereas a negative `wdiff` means that they wish to lose weight to achieve their desired weight.
...
```{r}
summary(wdiff)
hist(wdiff, breaks=200)
```

The distribution is unimodal and right skewed. There is a higer frequency of `wdiff` centered around 0. The data has 1st quartile of -21 and 3rd quartile of 0, therefore the IQR is 21. In all, this histogram suggests that there is a concentration of data from 0 to the negative side suggesting that a majority of the sample are okay with their actual weight or wish to lose their actual weight to achieve their desired weight.
...
##### Answer
```{r}
diffgen<-data.frame(cdc$gender,wdiff)
colnames(diffgen)[1]<-"gender"
aggregate(diffgen$wdiff~diffgen$gender,diffgen,summary)
boxplot(diffgen$wdiff~diffgen$gender)

# For a better view
boxplot(diffgen$wdiff[1:1000]~diffgen$gender[1:1000])
```

The summaries show that males tend to want to lose less weight than females when comparing their actual and desired weight differences. The median and mean for males are -5 and -10.7 respectively while females are -10 and -18.2 respectively.
...
# Proportion of the weights that are within 1 standard deviation of the mean, in percentage
...
# The proportion is
(sum(cdc$weight>= below & cdc$weight<= above)/20000)*100

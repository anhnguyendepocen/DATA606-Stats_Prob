...
This distribution is right skewed, as many of the values are accumulating around the median.
...
If a sample of the area data is taken each time, the histogram will change. It may appear normal if no high area values are in the set, but in general, this sample should carry the same distribution as the overall population, i.e. the distribution of area.
...
The mean of sample 1 and sample 2 will differ based on the same values included, as only about 1.7% of the population is analyzed. However, if the sampling size increases, the closer the sample mean will be to the population mean. So if the sample size was increased to 1000, it will provide a more accurate estimated of the population mean.
...
There are 5000 elements in `sample_means50`. The distribution appears normally distributed and centered around 1,500. If more sample means were found, the closer the distribution will get to normal. As a note, 5000 is already large, so a 50000 sample means would not show that large of a difference.
...
for (i in 1:100) {
  ...
  sample_means_small[i] <- mean(samp50)
}
sample_means_small
```

There are 100 element, and each element represents the mean of the sample population of area (n = 50).
...
When the sample size is larger, the sample statistics are closer to that of the true population, therefore, the center gets closer to the population mean and the spread gets narrower. 
...
```{r}
mean(price)
price_means50 <- sample(price,50)
summary(price_means50)
```
The mean of `price` is \$180,796.10, and from the summary statistics of the sample set, the best point estimate of the population mean would be the sample mean as it is closer in value then the median.
...
The distribution is approximately normal and centered at \$180,000. Given that this was a simulation of 5000 sample and that the population mean is \$180,796.10, this estimate is very close to the mean home price for the population.
...
Once again, the distribution is approximately normal and centered at \$180,000. Compared to the sampling distribution of a sample size of 50, these distribution match very closely and the estimate is once again very close to the mean home price for the population, ~\$180,000.
...
The sampling distribution from 3 has a narrower spread the compared to the sampling distribution from 2. To make estimates that are more often close to the  true value, a distribution with a small spread will provide closer estimations to the true population mean.

...
##### Answer
A streak length of 1 means that there was only one hit followed by a miss leading to the end of that streak. A streak length of 0 means that there was only a miss, no hits was made.
...
The distribution was right skewed. Kobe's streak lengths wasn't, honestly, impressive. The bar plot shows that he typical missed as there are more streak lengths of 0, and while he would make a hit some times, making a longer streak of 2 or more was rare. But he did managed to make a hit with the longest streak length of 4. 
...
##### Answer
```{r}
# The number of heads at showed up in the simulation is:
...
##### Answer
```{r}
# The changes needed to be made are the probablities of the outcome and the # of simulations
...
```{r}
# The streak length of 'sim_basket'
...
##### Answer
The distribution remains right skewed with the typical streak length of 0 for the simulated independent shooter. The longest streak of baskets in 133 shots is the max value:
...
##### Answer
The simulation will be somewhat similar in its distribution to the previous one, i.e. right shewed, each time it is run. There might be instances with longer streaks but it will remain a rare event given the probabilities. 
...
##### Answer
It is not evident that the hot hand model, where the probability of a hit increase after the first hit, fits Kobe's shooting patterns because the distribution of streak lengths are very similar for the steak length of Kobe and the simulated shooter, where each shot is independent to the next. Both the distribitions are right skewed with more occurances of streak length 0 and 1.

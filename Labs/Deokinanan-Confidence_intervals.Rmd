...
mean(population)
```
In general, the distribution of the sample often appears right-skewed. The "typical" size of house would be the sample mean, which is stated in the summary of the sample and the true population mean is 1499.69.
...
Not identical, but definitely similar since the samples are taken from the same population. The more we replicate this sampling and even increases the sample size, the more *identical* our distribution will become.
...
The sample must be independent and identically distributed, with more than 30 observation from a data that are not extremely skewed in either direction.
...
"95% confidence" means that we are 95% confident that the interval will have true population mean.
...
Yes, since the mean of the house sizes (1499.69) falls within the interval each time it was generated. Therefore, it is very likely that other student's interval will capture this value.
...
About 95% of the those intervals would capture the true population mean because if we take many samples and compute 95% confidence interval for each sample then about 95% of those confidence intervals will have true population mean.
...
From the plot, there is often an interval(s) highlighted in red that does not contained the population mean. I would suspect that on average 95% of these intervals will include the true mean. When it is less than 95%, it could be due to sampling error has it may contain too large or too small values that makes the mean deviates from true mean, affecting the interval.
...
Let the confidence level be 80%, this make alpha = 0.2 and critical p = 0.9.
```{r}
# The critical value will then be:
criticalval <- qnorm(0.9)
criticalval
```
...
```{r}
samp_mean60 <- rep(0,60)
samp_sd60 <- rep(0,60)
for (i in 1:50) {
  samp50 <- sample(population,50)
  samp_mean60[i] <- mean(samp50)
  samp_sd60[i] <- sd(samp50)
}
lower_vector <- samp_mean60 - criticalval * (samp_sd60/sqrt(60))
upper_vector <- samp_mean60 + criticalval * (samp_sd60/sqrt(60))
plot_ci(lower_vector,upper_vector,mean(population))
...
# The percentage of confidence intervals that contains the population mean is
(prop/50)*100
```

The percentage of confidence intervals that contains the population mean is given above. It was noted that each time the result was very close to 80%.

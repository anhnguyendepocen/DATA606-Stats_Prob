...
work space area in the upper right hand corner of the RStudio window now lists a 
...
Yes, there is a great increase from the start year of the records which then peaked at 1640 before it dramatically fall. It started to increase again but fell just before 1660. As the years went by, it began to raise, with a few drops, but nonetheless the trend kept increasing than in the earlier years. There are a few noticeable dips within this increase period such as a few years before and after 1700.
...
From the plot, the ratios are over 50%, meaning that there were more boys baptized than girls. There was a great peak from ~0.508 to ~0.535 during the years leading to 1660. The plot remains very spiky with increases and decreases year after year, with no apparent consistency.
...
The dataset contains birth records in the United States from 1940 to 2002. The data frame has 63 rows and 3 columns, with variable labels of `year`, `boys` and `girls`.
...
While these are both counts of newborn boys and girls, `Arbuthnot` data frame contains records of those baptized from early 17th century to the first decade of the 18th, 1629 to 1710, whereas `present` data frame is of newborn in general from 1940 to 2002. Therefore, `present` is a much larger scale than `Arbuthnot` as those numbers ranges in the million than the thousand as it is for `Arbuthnot`. If they were on the same timeline and from the same country, `Arbuthnot` data frame could have be said to be a subset of `present`.
...
title(main = "boy-to-girl ratio for every year")
```

From the plot, the calculation is over 50% for each record, meaning that there were more newborn boys than girls. This is consistent with the Arbuthnot's observation of the baptized newborns. There was a great drop from ~0.514 to ~0.512 during the years from around 1948 to 1951. There is great fluctuation from 1660 to 1670 as well. The plot continues to be very spiky with increases and decreases year after year.
...
present$year[which.max(total)]
max(total)
```

The most total number of births in U.S. was in 1961, with a total of 4,268,326 newborns.
